...
use crate::core::{Piece, PieceIndex, Hash, Bitfield, Result, TorrentError, FileError, ValidationError};
...
pub struct PieceManager {
    ...
    bitfield: Bitfield,
    /// Size of each piece (except possibly the last one)
    piece_length: u32,
    /// Total number of pieces
    num_pieces: usize,
    /// Cache of recently accessed pieces
    piece_cache: HashMap<PieceIndex, Vec<u8>>,
    /// Maximum number of pieces to keep in cache
    cache_size: usize,
}
...
impl PieceManager {
    ...
    /// Get a mutable reference to a piece
    ...
    pub async fn load_from_files(&mut self, file_paths: &[String], file_sizes: &[u64]) -> Result<()> {
        ...
        for piece_index in 0..self.num_pieces as PieceIndex {
            ...
            // Read piece data from multiple files if necessary
            while bytes_read < piece_size as usize && file_index < file_paths.len() {
                ...
                let mut file = File::open(file_path).await.map_err(|_e| {
                    TorrentError::File(FileError::NotFound { path: file_path.clone() })
                })?;
                ...
                let read = file.read(&mut piece_data[bytes_read..bytes_read + to_read]).await?;
                bytes_read += read;
    ...
    pub async fn write_to_files(&self, file_paths: &[String], file_sizes: &[u64]) -> Result<()> {
        ...
        for piece_index in 0..self.num_pieces as PieceIndex {
            ...
            let piece_data = self.get_piece_data(piece_index)
                .ok_or(TorrentError::File(FileError::NotFound { 
                    path: format!("piece {}", piece_index) 
                }))?;
            ...
            while bytes_written < piece_data.len() && file_index < file_paths.len() {
                ...
                let written = file.write(&piece_data[bytes_written..bytes_written + to_write]).await?;
